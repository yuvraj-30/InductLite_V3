    ...
    <section className="mt-6 rounded-lg border bg-white p-4">
      ...
      <form action={formAction} className="mt-4 space-y-3">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <label className="mt-1 inline-flex min-h-[44px] items-center gap-2 rounded-md border border-gray-200 px-3 text-sm text-gray-700">
            <input
              type="checkbox"
        ...
        <div className="mt-6 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs font-semibold uppercase tracking-[0.08em] text-gray-600">
                Print Profile
                <select
                  value={printProfile}
                  onChange={(event) =>
                  ...
                  className="ml-2 rounded-md border border-gray-300 px-2 py-1 text-xs font-medium text-gray-800"
                >
                ...
                {state.created.map((row) => (
                  <tr key={row.inviteId}>
                    <td className="px-3 py-2 text-sm text-gray-800">
                      ...
                      <p className="text-xs text-gray-500">Row {row.row}</p>
                    </td>
                    <td className="px-3 py-2 text-sm text-gray-700">{row.visitorType}</td>
                    ...
                    <td className="px-3 py-2 text-xs text-gray-700">
                      <button
                        ...
                        className="rounded border border-gray-300 px-2 py-1 font-semibold hover:bg-gray-50"
                      >
                        Copy Link
                      </button>
                    </td>
          ...
          >
            {state.created.map((row) => (
              ...
              >
                <p className="text-xs uppercase tracking-[0.08em] text-gray-500">Visitor</p>
                <p className="text-base font-semibold text-gray-900">{row.visitorName}</p>
                <p className="mt-1 text-xs text-gray-600">
                  {row.visitorType} - Expires {formatDateTime(row.expiresAt)}
                </p>
                <div className="mt-3 inline-flex rounded-md border border-gray-200 bg-white p-2">
                  {QRCodeSVGComponent ? (
                    <QRCodeSVGComponent
